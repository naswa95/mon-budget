...
function App() {
  ...
  return (
    <div className="app-shell">
      ...
      {modal?.type === "income" && <IncomeModal month={state.selectedMonth} onClose={() => setModal(null)} onSave={(income) => { updateProfile({ incomes: [...active.incomes, income] }); setModal(null); setToast("Revenu ajouté"); }} />}
...
function BudgetPage({ active, state, setState, totals, monthExpenses, monthIncomes, selectedDate, setModal }) {
  return <>
    ...
    <Calendar month={selectedDate} expenses={monthExpenses} incomes={monthIncomes} onExpense={(e) => setModal({ type: "expense", value: e })} />
  </>;
}
...
function Calendar({ month, expenses, incomes, onExpense }) {
  const y = month.getFullYear(), m = month.getMonth();
  const days = new Date(y, m + 1, 0).getDate();
  const offset = (new Date(y, m, 1).getDay() + 6) % 7;
  const cells = [...Array(offset).fill(null), ...Array.from({ length: days }, (_, i) => i + 1)];
  return <section className="calendar-card"><div className="weekdays">{["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"].map((d) => <span key={d}>{d}</span>)}</div><div className="calendar-grid">{cells.map((day, idx) => day === null ? <div className="day empty" key={`e-${idx}`} /> : <div className="day" key={day}><b>{day}</b><div className="day-items">{incomes.filter((i) => i.displayDate.getDate() === day).map((i) => <div className="event income" key={i.id}><span>{i.type}</span><strong>+{euro.format(i.amount)}</strong></div>)}{expenses.filter((e) => e.displayDate.getDate() === day).map((e) => <button className="event expense" key={e.id} onClick={() => onExpense(e)}><span>{e.name}</span><strong>{euro.format(e.amount)}</strong></button>)}</div></div>)}</div></section>;
}
